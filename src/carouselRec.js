...
export function renderRecommended() {
  ...
  items.forEach(item => {
    ...
    div.className = "recommended-card flex-none bg-gray-800 rounded-lg shadow-md overflow-hidden transform transition-transform duration-300 hover:scale-105 max-md:h-[16rem] max-md:w-[10rem] max-md:mr-3 md:h-[18rem] md:w-[12rem] md:mr-4 lg:mr-6 lg:h-[22rem] lg:w-[14rem] xl:h-[24rem] xl:w-[16rem]";;
    div.innerHTML = `
    <div class="img-container w-full h-full relative">
      ...
      <div class="recommended-title-overlay transition-all duration-300 max-md:px-1 max-md:py-2 md:py-[0.75rem] md:px-2">
        <h3 class="line-clamp-1 max-md:text-sm xl:text-base font-bold">${item.title}</h3>
      ...
      <div class="recommended-desc flex flex-col">
        ...
        <span class="text-sm max-lg:line-clamp-6 max-xl:line-clamp-10">${item.desc}</span>
        <a href="${item.link}" class="btn-comenzar flex content-center bg-black text-white hover:bg-white hover:text-black py-2 px-3 text-sm rounded-md absolute left-3 bottom-4"><img src="assets/play.svg" alt="play" class="mr-2"> ${item.start}</a>
      </div>
    </div>
  `;

    if (window.innerWidth < 768) {
      // Oculta la descripción, el blur, el zoom y mantiene visible el título en móvil
  ...
  function getCardWidth() {
    if (window.innerWidth >= 1800) { // xl
      return 18 * 16 + 24; // 16rem + mr-6 (1.5rem * 16px)
    } else if (window.innerWidth >= 1280) { // lg
      return 16 * 16 + 24; // 16rem + mr-6 (1.5rem * 16px)
    } else if (window.innerWidth >= 1024) { // lg
      return 14 * 16 + 24; // 14rem + mr-6
    } else if (window.innerWidth >= 768) { // md
      return 12 * 16 + 16; // 12rem + mr-4 (1rem * 16px)
    } else {
      return 10 * 16 + 12; // 10rem + mr-3 (0.75rem * 16px)
    }
  }
